    ...
    def getWeather(self):
        ...
        for element in data:
            if count==0 or count==5 or count==10:
                ...
                param_min.append(float(data_min[0].text.strip().split('°')[0].split('/\u2009')[1]))
                param_max.append(float(data_max[0].text.split('°')[0].strip()))
                param_rain.append(float(data_rain[0].text.split('l/')[0].strip()))
        ...
        for element in liste_son:
            if "Sonnenstunden" in element.text:
                param_sun.append(float(element.text.split("Sonnenstunden")[0].split("Freuen Sie sich auf bis zu")[1]))
